...
def read_crush_activity(
    ...
    try:
        ...
        rows = connection.execute(
            ...
            "WHERE overlap_rank <= ?), "
            "result_calls AS (SELECT DISTINCT new_page.session_id, "
            "json_extract(part.value, '$.data.tool_call_id') AS call_id "
            "FROM new_page JOIN json_each(CASE WHEN json_valid(new_page.parts) "
            "THEN new_page.parts ELSE '[]' END) AS part "
            ...
            "recent AS (SELECT * FROM overlap UNION SELECT * FROM new_page "

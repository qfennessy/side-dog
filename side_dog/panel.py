...
function bodyClass(){const autoColumns=innerWidth>=Math.max(1,state.roots.length)*300;const layout=state.layout==='auto'?(autoColumns?'columns':'stack'):state.layout;return layout+(state.paused?' paused':'')}
...
document.querySelectorAll('[data-layout]').forEach(b=>b.onclick=()=>{state.layout=b.dataset.layout;render()});document.querySelector('#expand').onclick=()=>{state.expanded=!state.expanded;document.querySelector('#expand').textContent=`e ${state.expanded?'compact':'expand'}`;render()};document.querySelector('#filter').onclick=()=>{state.filter={all:'milestones',milestones:'files',files:'all'}[state.filter];document.querySelector('#filter').textContent=`f ${state.filter}`;render()};document.querySelector('#pause').onclick=()=>{state.paused=!state.paused;document.querySelector('#pause').textContent=`p ${state.paused?'resume':'pause'}`;if(!state.paused){const q=state.queued.splice(0);q.forEach(apply)}render()};document.querySelector('#reverse').onclick=()=>{state.newest=!state.newest;document.querySelector('#reverse').textContent=`r ${state.newest?'oldest':'newest'}`;render()};document.querySelector('#all').onclick=()=>{state.focus=null;render()};
window.addEventListener('keydown',e=>{if(e.key==='a')state.focus=null;else if(e.key==='Tab'){e.preventDefault();const i=state.focus?state.roots.findIndex(r=>r.id===state.focus):-1;state.focus=state.roots[(i+1)%state.roots.length]?.id||null}else if(/^[1-9]$/.test(e.key))state.focus=state.roots[Number(e.key)-1]?.id||state.focus;else return;render()});

...
function latestHighwayCandidates(units,rootId){const selected=new Map();for(const unit of units){if(unit.root!==rootId)continue;for(const [index,event] of (unit.events||[]).entries()){const lane=highwayLane(event);if(!lane)continue;const epoch=Number(event.epoch_ms||unit.epoch||0);const key=event.operation_id?`operation:${event.operation_id}`:`unit:${unit.id}:${index}:${epoch}:${event.kind||''}`;const prior=selected.get(key);if(!prior||epoch>=prior.epoch)selected.set(key,{unit,event,lane,epoch,key})}}return[...selected.values()]}
function highwaySnapshot(units,rootId,nowMs,speed){const candidates=latestHighwayCandidates(units,rootId);const pixelsPerMs=HIGHWAY_BASE_PX_PER_MS*speed;const marks=candidates.map(({unit,event,lane,epoch,key})=>{const status=highwayStatus(event);const running=status==='running';const started=Number(event.started_epoch_ms||epoch);const age=Math.max(0,nowMs-epoch);return{id:`${unit.id}:${key}`,root:rootId,lane,status,judgment:highwayJudgment(status),epoch,y:running?0:Math.round(age*pixelsPerMs),hold:running?Math.max(4,Math.round(Math.max(0,nowMs-started)*pixelsPerMs)):0,detail:[event.title,event.detail].filter(Boolean).join(' · '),url:event.url||''}}).filter(mark=>mark.status==='running'||mark.y<=HIGHWAY_HEIGHT_PX);const stacks=new Map();for(const mark of marks){const bucket=`${mark.lane}:${Math.round(mark.y/8)}`;const index=stacks.get(bucket)||0;stacks.set(bucket,index+1);mark.offset=((index%5)-2)*10;mark.showJudgment=mark.status!=='success'||mark.y<24}let combo=0;for(const {event} of candidates.sort((a,b)=>a.epoch-b.epoch)){const status=highwayStatus(event);if(status==='success')combo+=1;else if(status==='failed')combo=0}return{marks,combo}}
...
const state={roots:[],units:new Map(),expanded:false,filter:'all',paused:false,newest:true,layout:'auto',focus:null,queued:[],view:'timeline',speed:1,motionReduced:motionQuery.matches,frozenAt:null};
...
function highwayMarkHTML(mark){const lane=HIGHWAY_LANES.indexOf(mark.lane);const fresh=mark.y<8?' fresh':'';const hold=mark.status==='running'?`<span class="hold"></span>`:'';const judgment=mark.showJudgment?`<span class="judgment">${mark.judgment}</span>`:'';const link=mark.url?`<a href="${esc(mark.url)}" target="_blank" rel="noopener" aria-label="${esc(mark.detail)}"></a>`:'';return`<span class="highway-note ${mark.status}${fresh}" style="--lane:${lane};--y:${mark.y}px;--hold:${mark.hold}px;--offset:${mark.offset}px" title="${esc(mark.detail)}" aria-label="${esc(mark.detail)}">${hold}${judgment}${link}</span>`}
function highwayHTML(root,nowMs){const snapshot=highwaySnapshot(visibleUnits(root),root.id,nowMs,state.speed);const lanes=HIGHWAY_LANES.map(lane=>`<div class="lane">${lane}</div>`).join('');return`<div class="highway-score"><span>pulse · ${state.speed}× · unknown stays neutral</span><span class="combo">combo ${snapshot.combo}</span></div><div class="highway" aria-label="Live activity highway for ${esc(root.name)}"><div class="lane-grid">${lanes}</div><div class="receptor"></div>${snapshot.marks.map(highwayMarkHTML).join('')}</div>`}
...
function renderHighways(nowMs){document.querySelectorAll('.highway-shell').forEach(shell=>{const root=state.roots.find(item=>item.id===shell.dataset.root);if(root)shell.innerHTML=highwayHTML(root,nowMs)})}
...
function toggleExpanded(){state.expanded=!state.expanded;document.querySelector('#expand').textContent=`e ${state.expanded?'compact':'expand'}`;render();showNotice(state.expanded?'Expanded history — grouped filesystem paths are open.':'Compact history — grouped filesystem paths are closed.')}
...
function togglePause(){state.paused=!state.paused;state.frozenAt=state.paused||state.motionReduced?Date.now():null;document.querySelector('#pause').textContent=`p ${state.paused?'resume':'pause'}`;if(!state.paused){const q=state.queued.splice(0);q.forEach(apply)}render();showNotice(state.paused?'Paused — collection continues; display updates are held.':'Live — held updates are now visible.')}
...
motionQuery.addEventListener('change',event=>{state.motionReduced=event.matches;state.frozenAt=state.paused||event.matches?Date.now():null;render();showNotice(event.matches?'Reduced motion — the pulse score is static and no animation frames run.':'Motion enabled — live highway movement is available.')});

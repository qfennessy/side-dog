...
class NativeAgentEventsTest(TestCase):
    ...
    def test_codex_backlog_does_not_guess_implicit_pr_heads(self) -> None:
        with TemporaryDirectory() as directory:
            root = (Path(directory) / "project").resolve()
            root.mkdir()
            state = Path(directory) / "state"
            ...
            with (
                ...
                patch("side_dog.cli._git_current_branch", return_value="beta"),
            ):
                count = poll_native_agent_events(
                    root,
                    identity,
    ...
    def test_codex_backlog_does_not_guess_implicit_merge_targets(self) -> None:
        with TemporaryDirectory() as directory:
            root = (Path(directory) / "project").resolve()
            root.mkdir()
            state = Path(directory) / "state"
            ...
            with (
                patch.dict(os.environ, {STATE_ENV: os.fspath(state)}),
                patch("side_dog.cli._git_current_branch") as current_branch,
                patch("side_dog.cli._git_push_default_target") as upstream,
            ):
                count = poll_native_agent_events(
                    root,
                    identity,
